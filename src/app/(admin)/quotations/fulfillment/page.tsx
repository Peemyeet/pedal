...
export default async function FulfillmentPage({
  ...
  searchParams?: Promise<FulfillmentSearchParams>;
}) {
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <h1 className="app-page-title">ยืนยันชำระเงิน</h1>
      ...
      <div className="space-y-4">
        {items.length === 0 ? (
          <p className="app-card px-6 py-14 text-center text-base text-[var(--muted)]">
            ยังไม่มีใบที่ชำระเงินและส่งของแล้ว
          </p>
        ) : (
          items.map((q) => {
            ...
            return (
              ...
              >
                ...
                <div className="mt-4 flex flex-wrap gap-2 text-sm">
                  <Link
                    href={`/quotations/${q.id}/quote`}
                    className="inline-flex rounded-full border border-indigo-300 bg-indigo-100 px-3 py-1.5 font-medium text-indigo-900 transition hover:bg-indigo-200"
                  >
                    เลขที่ใบเสนอราคา
                  </Link>
                  <Link
                    href={`/quotations/${q.id}?from=fulfillment`}
                    className={
                      paid
                  ...
                  <Link
                    href={sent ? `/quotations/${q.id}/postal` : `/quotations/${q.id}?from=fulfillment`}
                    className={
                      sent
