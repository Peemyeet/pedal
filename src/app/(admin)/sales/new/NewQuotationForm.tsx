...
import { createAndQuoteQuotation } from "@/app/(admin)/quotations/actions";
...
function NewQuotationFormInner({
  ...
  useEffect(() => {
    ...
    const deduped = uniqueLineRows(next);
    if (deduped.length) {
      setLines(deduped);
    }
  ...
  function addLine() {
    ...
    const p = products.find((x) => !selected.has(x.id));
    if (!p) return;
  ...
  function onProductChange(index: number, productId: string) {
    if (lines.some((row, i) => i !== index && row.productId === productId)) {
      return;
    }
                  ...
                  {products
                    .filter((p) => {
                      const usedByOther = lines.some(
                        (line, lineIndex) => lineIndex !== index && line.productId === p.id,
                      );
                      return !usedByOther || p.id === row.productId;
                    })
                    .map((p) => (
